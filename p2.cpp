...
//#define int int64_t
  ...
  case 'r': case 'R': {
    ...
    SimpleVector<num_t> b(::atoi(argv[2]));
    while(std::getline(std::cin, s, '\n')) {
    ...
    while(std::getline(std::cin, s, '\n')) {
      ...
      case 'l':
        for(i = 2; i < argc - 1; i ++)
          std::cout << in[::atoi(argv[i])] << ", ";
        std::cout << in[::atoi(argv[argc - 1])] << std::endl;
        break;
      case 't': case 'a': {
